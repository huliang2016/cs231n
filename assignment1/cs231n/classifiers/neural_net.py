...
class TwoLayerNet(object):
    ...
    def __init__(self, input_size, hidden_size, output_size, std=1e-4):
        ...
        std = math.sqrt(2.0 / float(input_size + hidden_size))
        a = math.sqrt(3.0) * std 
        self.params['W1'] = std * np.random.uniform(-a, a, size=(input_size, hidden_size))
        self.params['b1'] = np.zeros(hidden_size)

        std = math.sqrt(2.0 / float(hidden_size + output_size))
        a = math.sqrt(3.0) * std 
        self.params['W2'] = std * np.random.uniform(-a, a, size=(hidden_size, output_size))
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        corrects = scores[range(N), y]
        loss = corrects / scores_sums
        ...
        s[range(N), y] = - (scores_sums - corrects) / scores_sums
        s /= N
        # (hidden_size, N) * (N, output_size) = (hidden_size, output_size)
        dW2 = H1.T.dot(s)
        db2 = np.sum(s, axis=0)
        # (N, output_size) * (output_size, hidden_size) = (N, hidden_size)
        hidden = s.dot(W2.T)
        hidden[H1 == 0] = 0
        dW1 = X.T.dot(hidden)
